...
public class ThumbnailInsertController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			// 단, 여러개의 첨부파일이 있을 것이기 때문에 Attachment
			...
			for(int i=1; i<=4; i++) {
				...
				if(multiRequest.getOriginalFileName(key) != null) {
					// 첨부파일이 존재할 경우
					// Attachment생성 + 원본명,수정명,폴더경로,파일레벨 담아서 => list에 쌓기
			...
			if(result > 0) { // 성공 => list.th 재요청 => 사진게시판리스트페이지
				request.getSession().setAttribute("alertMsg", "사진게시판 등록 성공");
				response.sendRedirect("list.th");
			} else { // 실패 => 에러페이지
				request.getSession().setAttribute("errorMsg", "불러오기 실패했어요~");
