...
public class BoardListController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int boardLimit;		// 한 페이지 내에 보여질 게시글 최대 개수 (몇 개 단위씩)
		...
		//System.out.println(listCount); // 중간점검 : 107!!
		
		// * currentPage : 현재 페이지 (즉, 사용자가 요청한 페이지)
		currentPage = Integer.parseInt(request.getParameter("currentPage"));
		
		// * pageLimit : 하단에 보여질 페이징바의 페이지 최대 개수 (페이지 목록들 몇 개 단위로?)
		pageLimit = 10;
		...
		// startPage가 11이여서 endPage가 20이 됨 (근데 maxPage가 고작 13까지 밖에 안된다면?)
		// => endPage를 maxPage로 변경
		if(endPage > maxPage) {
			endPage = maxPage;
		}
		
		// 페이지 정보들을 하나의 공간에 담아서 보내자!! => vo클래스 만들어버리기
		// 1. 페이징 바 만들때 필요한 객체
		...
		if(list != null) { // 성공
			...
			request.getSession().setAttribute("errorMsg", "작성된 게시물이 없습니다.");
		}
